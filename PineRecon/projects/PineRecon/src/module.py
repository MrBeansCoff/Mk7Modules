...
class HandshakeCaptureJob(Job[dict]):
    ...
    def do_work(self, logger: Logger) -> dict:
        ...
        deauth_command = self._deauth_command()
        ...
        with open(self.log_path, 'w') as stdout_file:
            ...
            if deauth_command:
                time.sleep(1)
                stdout_file.write(f'PineRecon deauth command: {" ".join(deauth_command)}\n\n')
                stdout_file.flush()
                self.deauth_proc = subprocess.Popen(deauth_command, stdout=stdout_file, stderr=stdout_file, start_new_session=True)
            try:
        ...
        result['deauth_engine'] = 'aireplay-ng' if deauth_command else ''
    ...
    def _stop_deauth(self):
        if self.deauth_proc and self.deauth_proc.poll() is None:
            try:
                os.killpg(os.getpgid(self.deauth_proc.pid), signal.SIGTERM)
            except Exception:
            ...
            try:
                ...
                try:
                    os.killpg(os.getpgid(self.deauth_proc.pid), signal.SIGKILL)
                except Exception:
...
def _load_ouis() -> None:
    ...
    path = pathlib.Path(OUI_FILE)
    if not path.exists() or not path.is_file():
        OUI_ERROR = f'OUI database not found at {OUI_FILE}.'
    ...
    try:
        ...
        OUI_ERROR = f'No OUI records found in {OUI_FILE}.'
        module.logger.warning(OUI_ERROR)
...
def _dependency_state() -> dict:
    ...
    package_state['aireplay-ng'] = _has_named_tool('aireplay-ng')
...
def _busy_capture_processes(interface: str = '') -> List[dict]:
    process_names = ['hcxdumptool', 'airodump-ng', 'aireplay-ng', 'tcpdump']
    busy = []
...
def _handshake_preflight(interface: str, channel: str = '') -> dict:
    ...
    if not _has_named_tool('aireplay-ng'):
        warnings.append('aireplay-ng is unavailable; handshake capture will be passive only.')
...
def status(request: Request):
    ...
    return {
        ...
        'has_aireplay': dependencies['package_state'].get('aireplay-ng', False),
