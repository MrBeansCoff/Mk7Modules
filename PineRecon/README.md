...
- OUI lookup using Pineapple and ieee-data OUI sources when present on the device.
...
- Capture preflight checks for interface state, active capture jobs, monitor mode, and selected AP channel.
...
OUI lookup sources checked by the backend:
...
| `handshake_preflight` | Checks capture engine, interface availability, monitor state, active jobs, and target channel warnings. |
| `start_handshake_capture` | Tunes the selected interface to the target channel and starts a timed handshake capture job. |
| `stop_handshake_capture` | Stops the active capture job. |
| `start_pineap_handshake` | Starts Pine DAP using the original PineAP handshake capture command. |
...
PineRecon keeps the raw capture file available for download. The validation status is only a confidence indicator; the downloaded `.pcapng` or `.cap` can still be processed with external tooling such as `hcxpcapngtool`, Hashcat, or Aircrack-ng.
...
Pine DAP is available inside the Handshakes tab as an original PineAP handshake option.
